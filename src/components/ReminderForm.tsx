...
import { Pill, Calendar, Clock, Send, BellRing, AlertCircle, CheckCircle, Mail } from 'lucide-react';
...
export const ReminderForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({
        type: 'success',
        text: 'Đặt lịch nhắc nhở thành công! Hệ thống sẽ gửi email thông báo khi đến giờ hẹn.',
      });
      ...
      <div className="flex items-center gap-2.5 pb-3 border-b border-neutral-200/40">
        ...
        <div>
          ...
          <span className="text-[9px] font-bold uppercase tracking-wider text-neutral-500 mt-1 block">
            Gửi email thông báo tự động qua hòm thư
          </span>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[10px] font-bold uppercase tracking-wider flex items-center gap-1.5 text-neutral-500">
              <Mail size={12} className="text-red-500" /> Email nhận thông báo
            </label>
            <input
              type="email"
              required
            ...
            <span className="text-[9px] text-neutral-500 font-medium leading-relaxed pl-1">
              Nhập địa chỉ Email cá nhân để nhận cảnh báo định kỳ từ hệ thống.
            </span>
          </div>
